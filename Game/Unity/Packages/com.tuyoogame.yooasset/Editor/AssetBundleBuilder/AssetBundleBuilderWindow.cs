    ...
    public class AssetBundleBuilderWindow : EditorWindow
    {
        ...
        public void CreateGUI()
        {
            try
            {
                ...
                // 构建包裹
                {
                    string packageName = AssetBundleBuilderSetting.GetCurrentBuildPackageName();
                    _buildPackage = packageNames.Contains(packageName) ? packageName : packageNames[0];
                    _packageMenu = new ToolbarMenu();
                    _packageMenu.style.width = 200;
                    foreach (var name in packageNames)
                    {
                        _packageMenu.menu.AppendAction(name, PackageMenuAction, PackageMenuFun, name);
                    }
        ...
        private void PackageMenuAction(DropdownMenuAction action)
        {
            ...
            if (_buildPackage != packageName)
            {
                ...
                AssetBundleBuilderSetting.SetCurrentBuildPackageName(_buildPackage);
                RefreshBuildPipelineView();
            }
        }
